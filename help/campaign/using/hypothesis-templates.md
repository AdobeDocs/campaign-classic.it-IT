...
1. Fai clic su **[!UICONTROL New]** o fai clic con il pulsante destro del mouse nell’elenco dei modelli e scegli **[!UICONTROL New]** nell’elenco a discesa.
...
1. Per i modelli di tipo **[!UICONTROL Delivery]**, specifica se le misurazioni devono essere eseguite con o senza un gruppo di controllo (per ulteriori informazioni, consulta [Proprietà di un modello di ipotesi](#properties-of-a-hypothesis-template)).
1. Per i modelli di tipo **[!UICONTROL Delivery]**, puoi scegliere un canale specifico o decidere di applicare il modello a tutti i canali disponibili in Adobe Campaign utilizzando l&#39;elenco a discesa **[!UICONTROL Channel]** (per ulteriori informazioni, consulta [Proprietà di un modello di ipotesi](#properties-of-a-hypothesis-template)).
1. Seleziona il **[!UICONTROL Execution folder]** in cui desideri creare ed eseguire automaticamente le ipotesi che verranno create da questo modello.
1. Scegli le impostazioni di esecuzione (per ulteriori informazioni, consulta [Impostazioni di esecuzione del modello di ipotesi](#hypothesis-template-execution-settings)).
1. Specifica il periodo di calcolo dell&#39;ipotesi (per ulteriori informazioni, consulta [Impostazioni di esecuzione del modello di ipotesi](#hypothesis-template-execution-settings)).

   >[!CAUTION]
   >
   >Questo periodo è determinato dalla data di contatto.

1. Nella scheda **[!UICONTROL Transactions]** , specifica le tabelle e i campi necessari per il calcolo dell’ipotesi (per ulteriori informazioni, consulta [Transazioni](#transactions)).
1. Se il modello è configurato per le ipotesi di tipo **[!UICONTROL Offer]**, puoi abilitare l&#39;opzione **[!UICONTROL Update offer proposition status]** : in questo caso, seleziona lo stato della proposta di offerta da modificare.
1. Specificare l&#39;ambito dell&#39;applicazione dell&#39;ipotesi (per ulteriori informazioni, fare riferimento a [perimetro dell&#39;ipotesi](#hypothesis-perimeter)).
1. Se necessario, utilizza uno script per completare il filtro (per ulteriori informazioni, fai riferimento a [Perimetro di ipotesi](#hypothesis-perimeter)).
...
* **[!UICONTROL Measurement with control group]**: consente di specificare se un gruppo di controllo è stato definito per la consegna o la campagna e di includerlo negli indicatori di misurazione. Il gruppo di controllo, che non riceve consegne, ti consente di misurare l’impatto della campagna dopo la consegna, confrontandola con la popolazione target che ha ricevuto la consegna.
   ...
   Per ulteriori informazioni sulla definizione e la configurazione di un gruppo di controllo, consulta [Definizione di un gruppo di controllo](../../campaign/using/marketing-campaign-deliveries.md#defining-a-control-group).

* **[!UICONTROL Channel]**: puoi scegliere un canale specifico o rendere il modello di ipotesi disponibile per tutti i canali nella console Adobe Campaign selezionando  **[!UICONTROL All channels]** nell’elenco a discesa. Se configuri il modello per un canale specifico, questo ti consente di filtrare automaticamente le consegne per canale durante la creazione dell’ipotesi (consulta [Creazione di ipotesi](../../campaign/using/creating-hypotheses.md)).
...
* **[!UICONTROL Automatic execution]**: se necessario, ti consente di pianificare il ricalcolo delle ipotesi (ad esempio, se desideri aggiornare regolarmente gli indicatori fino alla fine della consegna).
   ...
   1. Fai clic sul collegamento **[!UICONTROL Frequency of execution...]** , quindi sul pulsante **[!UICONTROL Change...]** .
...
* **[!UICONTROL Measurement period]**: consente di configurare le date di inizio e di fine durante le quali vengono eseguite le ipotesi e le linee di acquisto vengono recuperate.
   ...
   Se l&#39;ipotesi viene lanciata al volo, può essere forzata se vuole attivarla immediatamente. In caso contrario, viene attivato automaticamente in base alla data di fine del calcolo configurata, basata sulla data di creazione dell&#39;ipotesi (consulta [Creazione rapida di un&#39;ipotesi su una consegna](../../campaign/using/creating-hypotheses.md#creating-a-hypothesis-on-the-fly-on-a-delivery)).

* **[!UICONTROL Transaction/Margin amount]**: questi campi sono facoltativi e consentono di calcolare automaticamente gli indicatori del fatturato (consulta  [Indicatori](../../campaign/using/hypothesis-tracking.md#indicators)).
* **[!UICONTROL Unit amount]**: consente di impostare un importo per il calcolo dei ricavi (consulta  [Indicatori](../../campaign/using/hypothesis-tracking.md#indicators)).
...
* **Filtro sulle transazioni**: nella  **[!UICONTROL Scope]** scheda puoi configurare un filtro per l’ipotesi. Per eseguire questa operazione:
...
* **Filtrare i destinatari**: nella  **[!UICONTROL Scope]** scheda , puoi limitare l’ipotesi a qualsiasi informazione collegata a un messaggio (consegna, destinatario, indirizzo e-mail, servizio, ecc.):

   1. Fai clic sul collegamento **[!UICONTROL Add a filter]**, quindi **[!UICONTROL Edit query]**.
...
* **Script**: è possibile utilizzare uno script JavaScript per sovraccaricare dinamicamente le impostazioni delle ipotesi durante la relativa esecuzione.
   ...
   A questo scopo, fai clic sul collegamento **[!UICONTROL Advanced settings]** , quindi immetti lo script desiderato.
...
In questo esempio, creeremo un modello di ipotesi su una consegna di tipo direct mailing. La tabella delle transazioni (**Acquisti** nel nostro esempio) su cui si baseranno le ipotesi contiene le linee di acquisto collegate agli articoli o ai prodotti. Vogliamo configurare il nostro modello per creare ipotesi su articoli o prodotti nella tabella acquisti.

1. Nell’esploratore Adobe Campaign, passa al nodo **[!UICONTROL Resources > Templates > Hypothesis templates]** .
...
1. Nella scheda **[!UICONTROL Transactions]** , seleziona la tabella delle reazioni dei destinatari.

   ![](assets/response_hypothesis_model_example_006.png)

1. Nel campo **[!UICONTROL Transactions schema]** , scegli la tua tabella di acquisto.
...
1. Nella scheda **[!UICONTROL Scope]** , fai clic su **[!UICONTROL Edit query]** per creare un filtro per le ipotesi.
