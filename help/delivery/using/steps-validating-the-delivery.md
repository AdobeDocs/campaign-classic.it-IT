...
1. Per interrompere l’analisi in qualsiasi momento, fai clic su **[!UICONTROL Stop]**.
...
1. Fai clic su **[!UICONTROL Close]** correggere gli eventuali errori.

1. Dopo aver apportato le modifiche, riavvia l&#39;analisi facendo clic su **[!UICONTROL Analyze]**.

Dopo aver controllato il risultato dell’analisi, potrai fare clic su **[!UICONTROL Confirm delivery]** per inviare il messaggio alla destinazione specificata. Un messaggio di conferma consente di avviare la consegna.

![](assets/s_ncs_user_email_del_analyze_ok.png)

>[!NOTE]
>
>Fai clic sul pulsante **[!UICONTROL Change the main delivery target]** se il numero di messaggi da inviare non corrisponde alla configurazione. Ciò ti consente di modificare la definizione della popolazione target e di riavviare l’analisi.

### Impostazioni di analisi {#analysis-parameters}

La **[!UICONTROL Analysis]** la scheda delle proprietà di consegna ti consente di definire un set di informazioni relative alla preparazione dei messaggi durante la fase di analisi.
...
Per accelerare la preparazione della consegna, puoi controllare la **[!UICONTROL Prepare the delivery parts in the database]** prima di avviare l’analisi.
...
* Non è necessario utilizzare il routing di mid-sourcing o esterno, solo il tipo di indirizzamento di consegna in serie. Puoi controllare il ciclo utilizzato nella **[!UICONTROL General]** della scheda **[!UICONTROL Delivery properties]**.
* Non è possibile eseguire il targeting di una popolazione proveniente da un file esterno. Per una singola consegna, fai clic sul pulsante **[!UICONTROL To]** collegamento dal **[!UICONTROL Email parameters]** e controlla che **[!UICONTROL Defined in the database]** è selezionata. Per una consegna utilizzata in un flusso di lavoro, verifica che i destinatari siano **[!UICONTROL Specified by the inbound event(s)]** in **[!UICONTROL Delivery]** scheda .
* È necessario utilizzare un database PostgreSQL.

### Configura la priorità dell’analisi {#analysis-priority-}

Quando la consegna fa parte di una campagna, la **[!UICONTROL Advanced]** tab offre un’opzione aggiuntiva. In questo modo puoi organizzare l’ordine di elaborazione delle consegne nella stessa campagna.
...
Le opzioni per **[!UICONTROL Message preparation by the scheduler]** ti consente di assegnare priorità all’analisi della consegna in un flusso di lavoro della campagna.
...
>[!NOTE]
>
>Per evitare che le analisi di consegna più grandi rallentino l’avanzamento dei flussi di lavoro, puoi pianificare le esecuzioni selezionando il pulsante **[!UICONTROL Schedule execution for a time of low activity]**.
...
Una volta analizzata la consegna, puoi visualizzare le varie bozze inviate tramite il **[!UICONTROL Proofs]** sottoscheda del registro (**[!UICONTROL Audit]** ).
...
La **[!UICONTROL Advanced]** La scheda delle proprietà di consegna ti consente di definire le proprietà della bozza. Se necessario, puoi sovrascrivere le regole di esclusione dei destinatari.
...
* Entrambe le opzioni seguenti consentono di tenere in quarantena i destinatari che sono elenco Bloccati e gli indirizzi. Vedi la descrizione di queste opzioni per il target principale in [Personalizzare le impostazioni di esclusione](steps-defining-the-target-population.md#customizing-exclusion-settings). A differenza del target di una consegna, in cui questi indirizzi sono esclusi per impostazione predefinita, vengono mantenuti per impostazione predefinita per il target di una bozza.
* La **[!UICONTROL Keep the delivery code for the proof]** consente di assegnare alla bozza lo stesso codice di consegna definito per la consegna a cui si riferisce. Questo codice è specificato nel primo passaggio della procedura guidata di consegna.
* Per impostazione predefinita, l’oggetto della bozza è preceduto da &quot;Proof #&quot;, dove # è il numero della bozza. Puoi modificare questo prefisso nella **[!UICONTROL Label prefix]** campo .

## Processo di convalida con tipologie {#validation-process-with-typologies}

Prima di inviare qualsiasi messaggio, è necessario analizzare la campagna per approvarne il contenuto e la configurazione. Le regole di controllo applicate durante la fase di analisi sono definite in un **tipologia**. Per impostazione predefinita, per le e-mail l’analisi riguarda i seguenti punti:
...
La tipologia da applicare per ogni consegna è selezionata nella **[!UICONTROL Typologies]** nei parametri di consegna.

Puoi visualizzare e modificare le regole di approvazione, il loro contenuto, l’ordine di esecuzione e la descrizione completa tramite la **[!UICONTROL Administration > Campaign execution > Typology management > Typology rules]** nodo.
...
Per modificare la tipologia corrente, fai clic sul pulsante **[!UICONTROL Edit link]** a destra della **[!UICONTROL Typology]** campo .

![](assets/s_ncs_user_email_del_typo_tab.png)

La **[!UICONTROL Rule]** fornisce un elenco delle regole di tipologia da applicare. Seleziona una regola e fai clic sul pulsante **[!UICONTROL Detail...]** icona per visualizzarne la configurazione:
