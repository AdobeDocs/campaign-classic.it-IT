...
La scelta del dominio per un DNS inverso ha un impatto quando si tratta di alcuni ISP. AOL, in particolare, accetta solo cicli di feedback con un indirizzo nello stesso dominio del DNS inverso (vedere [Ciclo di feedback](#feedback-loop)).
...
>[!NOTE]
>
>* Se hai configurato DomainKeys per la tua istanza  Adobe Campaign, devi solo selezionare **dkim** nelle [regole di gestione del dominio](../../delivery/using/understanding-delivery-failures.md#domain-management). In caso contrario, seguire gli stessi passaggi di configurazione (chiave pubblica/privata) di DomainKeys.
...
* Create o modificate una regola di posta indesiderata, **Feedback_loop**, in **[!UICONTROL Administration > Campaign Management > Non deliverables Management > Mail rule sets]** con il motivo **Rifiutato** e il tipo **Rigido**.
...
La seguente riga di comando può essere utilizzata per creare una **List-Unsubscription** dinamica:
...
* aggiunta diretta della riga di comando nel modello di consegna - vedere [questa sezione](#adding-a-command-line-in-a-delivery-template),
* oppure, creazione di una regola di tipologia - vedere [questa sezione](#creating-a-typology-rule).
...
1. Annulla sottoscrizione elenco: &lt;mailto:unsubscribe@domain.com>
   ...
   Facendo clic sul collegamento **unsubscription** si apre il client di posta elettronica predefinito dell&#39;utente. Questa regola di tipologia deve essere aggiunta in una tipologia utilizzata per creare e-mail.

1. Annulla sottoscrizione elenco: `<https://domain.com/unsubscribe.jsp>`

   Facendo clic sul collegamento **unsubscription** l&#39;utente viene reindirizzato al modulo di annullamento dell&#39;iscrizione.
