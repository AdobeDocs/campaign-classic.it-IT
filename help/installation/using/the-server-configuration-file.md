...
>[!NOTE]
>
>Le configurazioni lato server possono essere eseguite solo da Adobe per le distribuzioni ospitate da Adobe. Per ulteriori informazioni sulle diverse implementazioni, consulta [Modelli di hosting](../../installation/using/hosting-models.md) sezione o [questa pagina](../../installation/using/capability-matrix.md). I passaggi di installazione e configurazione per i modelli in hosting e ibridi sono descritti in questo [sezione](../../installation/using/hosting-models.md).

I primi parametri si trovano all’interno del **condiviso** nodo. Sono correlati all’istanza. Sono potenzialmente utilizzati da tutti i comandi nlserver (nlserver web, nlserver wfserver, ecc.). Le altre sezioni sono correlate a un sottocomando nlserver specifico.
...
Di seguito sono riportati i diversi parametri del **autenticazione** nodo:
...
Di seguito sono riportati i diversi parametri del **authentication > XTK** nodo:
...
Di seguito sono riportati i diversi parametri del **dataStore** nodo. In questo punto vengono definite le origini dati del server.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> extraSandboxDirectory<br /> </td> 
  ...
  <tr> 
   <td> host<br /> </td> 
   <td> Maschere DNS: elenco di maschere DNS utilizzate da questa istanza (separate da virgole, è possibile utilizzare * e ? modelli).<br /> </td> 
   <td> Stringa<br /> </td> 
   <td> '*'<br /> </td> 
  </tr> 
  <tr> 
...
Di seguito sono riportati i diversi parametri del **dataStore > dataSource** nodo.
...
In **dataStore > dataSource > dbcnx** nodo, configurare le impostazioni di connessione:
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> fornitore<br /> </td> 
   <td> Tipo (enumerazione). I valori possibili sono "Oracle", "MSSQL" (Microsoft SQL Server), "PostgreSQL" (PostgreSQL), "Teradata", "DB2", "MySQL", "Netezza", "AsterData", "SAPHANA" (SAP HANA), "RedShift" (Amazon Redshift), "ODBC" (ODBC (Sybase, Sybase IQ)), Relay' (inoltro HTTP al database remoto).<br /> </td> 
   <td> Stringa<br /> </td> 
   <td> "Oracle"<br /> </td> 
  </tr> 
  <tr> 
...
In **dataStore > dataSource > sqlParams** nodo, configurare i parametri SQL:
...
In **dataStore > dataSource > pool** nodo, configurare i parametri del pool di connessioni associato:
...
Di seguito sono riportati i diversi parametri del **dataStore > virtualDir** nodo. Questa è la configurazione della directory virtuale alla mappatura della directory reale.
...
Di seguito sono riportati i diversi parametri del **dataStore > preprocessCommand** nodo. Si tratta dei comandi autorizzati per la pre-elaborazione dell’attività del flusso di lavoro &quot;Load file&quot;.
...
Di seguito sono riportati i diversi parametri del **dnsConfig** (Configurazione DNS).

Per ulteriori informazioni, consulta [sezione](../../installation/using/configuring-campaign-server.md).
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> Server DNS: elenco separato da virgole dei server dei nomi di dominio (DNS). Vedi la nota qui sotto.<br /> </td> 
   <td> Stringa<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
Di seguito sono riportati i diversi parametri del **exec** (esecuzione del comando) nodo.
...
Di seguito sono riportati i diversi parametri del **ims** nodo. Configurazione per la connessione di Campaign a un altro servizio tramite [IMS](../../integrations/using/about-adobe-id.md).
...
Di seguito sono riportati i diversi parametri del **mailExchanger** nodo. Configurazione del server SMTP.
...
Di seguito sono riportati i diversi parametri del **modulo** nodo. Questa è la configurazione per il modulo xtk delle restrizioni dei namespace.
...
Di seguito sono riportati i diversi parametri del **monitoraggio** nodo. Questa è la configurazione del servizio di monitoraggio.
...
Di seguito sono riportati i diversi parametri del **ooconv** nodo. Configurazione del server di conversione documenti.
...
Di seguito sono riportati i diversi parametri del **proxyConfig** nodo. Questa è la configurazione dei parametri proxy.
...
Di seguito sono riportati i diversi parametri del **threadPool** nodo.
...
Di seguito sono riportati i diversi parametri del **urlPermission** nodo. Questo è l&#39;elenco di URL a cui il codice JavaScript può accedere.
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> suffisso dns<br /> </td> 
   <td> Nome di dominio o padre di dominio interessato dall’URL: tutto o parte del dominio dell'URL per verificare, accelerare la verifica. L’URL viene verificato rispetto all’espressione regolare solo se il relativo dominio contiene dsnSuffix.<br /> </td> 
   <td> Stringa<br /> </td> 
  </tr> 
  <tr> 
   <td> urlRegEx<br /> </td> 
...
Se un record soddisfa **suffisso dns** ma non **urlRegEx**, viene esaminato il seguente record.
...
Di seguito sono riportati i diversi parametri del **xtkJobs** nodo. Configurazione dei processi server.
...
Di seguito sono riportati i diversi parametri del **archiviazione** nodo. Si tratta della configurazione delle operazioni di archiviazione eseguite in background.
...
Di seguito sono riportati i diversi parametri del **inMail** nodo. Questa è la configurazione del modulo di gestione delle e-mail in entrata.
...
In **inMail > msgDump** configura i seguenti parametri. Questa è la configurazione del dump dei messaggi elaborati.
...
Di seguito sono riportati i diversi parametri del **interattivo** nodo. Configurazione del daemon di scrittura per gli eventi di interazione in entrata.
...
Di seguito sono riportati i diversi parametri del **mta** nodo. Questa è la configurazione degli agenti di consegna.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> notificaRelay<br /> </td> 
   <td> Relè di notifica: Nome host:porta utilizzata per le notifiche di inoltro.<br /> </td> 
   <td> Stringa<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> Se è impostato su "true", l'istanza utilizza la variabile <a href="../../delivery/using/sending-with-enhanced-mta.md" target="_blank">MTA avanzato</a>.<br /> </td> 
   <td> Booleano<br /> </td> 
   <td> <br /> </td>b 
  </tr>
  <tr> 
...
In **cache** configura i seguenti parametri. Questa è la configurazione della cache del file locale.
...
In **mta > relè** configura i seguenti parametri. Configurazione del server di posta per la consegna del messaggio.
...
Per ulteriori informazioni, consulta [relè SMTP](../../installation/using/configuring-campaign-server.md#smtp-relay).
...
In **mta > master** configura i seguenti parametri. Questa è la configurazione del server principale.

Per ulteriori informazioni, consulta [sezione](../../installation/using/configuring-campaign-server.md#mta-child-processes).
...
In **mta > figlio** configura i seguenti parametri. Configurazione dei server figlio.
...
In **mta > figlio > smtp > IPAffinity** configura i seguenti parametri. Si tratta della configurazione della gestione delle affinità con indirizzi IP per il traffico SMTP in uscita ottimizzato.
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> localDomain<br /> </td> 
   <td> Nome di dominio: nome di dominio locale collegato all'indirizzo IP. Utilizzato quando si emette un comando SMTP HELO.<br /> </td> 
   <td> Stringa<br /> </td> 
  </tr> 
  <tr> 
   <td> name<br /> </td> 
...
Di seguito sono riportati i diversi parametri del **nmac** nodo. Configurazione di per le consegne di notifiche push.
...
Di seguito sono riportati i diversi parametri del **nmac > relè** nodo. Questo configura l’utilizzo di un relay per la consegna del messaggio (connettore ios http2).
...
Di seguito sono riportati i diversi parametri del **condutturato** nodo. Questa è la configurazione del modulo di elaborazione degli eventi per Pipeline Services.
...
Di seguito sono riportati i diversi parametri del **riparazione** nodo. Questa è la configurazione del modulo di ripristino del database.
...
Di seguito sono riportati i diversi parametri del **securityZone** nodo.
...
Di seguito sono riportati i diversi parametri del **securityZone > subNetwork** nodo.
...
Di seguito sono riportati i diversi parametri del **sms** nodo. Questa è la configurazione del modulo di gestione SMS in entrata.
...
Di seguito sono riportati i diversi parametri del **sms > netsize** nodo.
...
Di seguito sono riportati i diversi parametri del **stat** nodo. Questa è la configurazione del modulo statistiche MTA.
...
Di seguito sono riportati i diversi parametri del **syslogd** nodo. Questa è la configurazione del modulo di gestione del registro.
...
Di seguito sono riportati i diversi parametri del **trackinglogd** nodo. Questa è la configurazione del daemon di registrazione del tracking.
...
Di seguito sono riportati i diversi parametri del **web** nodo. Questa è la configurazione del modulo Web.

Per ulteriori informazioni, consulta [sezione](configuring-campaign-server.md#default-port-for-tomcat).
...
Di seguito sono riportati i diversi parametri del **web > jsp** nodo. Si tratta della configurazione dei parametri utilizzati dai JSP.
...
La **web > jsp > classpath** node contiene l&#39;elenco di tutti i percorsi di classe da utilizzare all&#39;avvio di JVM. Di seguito è riportata la configurazione predefinita:
...
Di seguito sono riportati i diversi parametri del **web > jssp** nodo. Si tratta della configurazione dei parametri utilizzati dai JSSP.
...
La **web > jsp > classpath** node contiene l&#39;elenco di tutti i percorsi di classe da utilizzare all&#39;avvio di JVM.

### relè {#relay-2}

Di seguito sono riportati i diversi parametri del **web > relè** nodo. Questa è la configurazione del relay per le richieste HTTP tra due aree.

Per ulteriori informazioni, consulta [sezione](../../installation/using/deploying-an-instance.md#synchronizing-public-resources).
...
Aggiungi un **web > relay > url** nodo per ogni URL da inoltrare (l&#39;ordine di inserimento definisce la priorità) con i seguenti parametri.

Per ulteriori informazioni, consulta [Sicurezza e relè delle pagine dinamiche](../../installation/using/configuring-campaign-server.md#dynamic-page-security-and-relays) e [sezione](../../installation/using/deploying-an-instance.md#synchronizing-public-resources).
...
Aggiungi un **web > relay > responseHeader** nodo per ogni intestazione HTTP da aggiungere alle risposte inoltrate al relay.
...
Di seguito sono riportati i diversi parametri del **web > reindirizzamento** nodo. Questa è la configurazione del modulo di reindirizzamento.
...
Di seguito sono riportati i diversi parametri del **web > reindirizzamento > spareServer** nodo.
...
Di seguito sono riportati i diversi parametri del **web > spamCheck** nodo. Configurazione dei parametri di valutazione dell’anti-spam e-mail.
...
Di seguito sono riportati i diversi parametri del **wfserver** nodo. Questa è la configurazione del processo del flusso di lavoro.
