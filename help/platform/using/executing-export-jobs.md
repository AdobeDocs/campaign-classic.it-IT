...
description: Scoprite come configurare ed eseguire i processi di esportazione in Campaign Classic.
audience: platform
...
# Configurazione dei processi di esportazione {#executing-export-jobs}

I processi di esportazione consentono di accedere ed estrarre dati dal database: contatti, client, elenchi, segmenti, ecc.

Ad esempio, può essere utile utilizzare i dati di tracciamento campagna (cronologia di tracciamento, ecc.) in un foglio di calcolo. I dati di output possono essere in formato testo, CSV, TAB o XML.
...
La procedura guidata di esportazione viene visualizzata dopo la creazione di un nuovo processo di esportazione (vedere [Creazione di processi di importazione ed esportazione](../../platform/using/creating-import-export-jobs.md).
...
* Fare clic su **[!UICONTROL Add a calculated field]** se si desidera creare un nuovo campo calcolato (ad esempio, una colonna contenente cognome + nome). Per ulteriori informazioni, fare riferimento a [Campi calcolati](../../platform/using/executing-import-jobs.md#calculated-fields).
