...
La piattaforma Adobe Campaign è accessibile tramite una console o un browser Internet. Vedi i browser compatibili in [matrice di compatibilità](../../rn/using/compatibility-matrix.md#Browsers).
...
Ulteriori informazioni su [utilizzo dell’interfaccia web](../../campaign/using/accessing-marketing-campaigns.md#using-the-web-interface-).
...
Ogni funzionalità include una serie di funzionalità basate su esigenze correlate alle attività e sul contesto di utilizzo. Ad esempio, **[!UICONTROL Profiles and targets]** link ti consente di accedere agli elenchi dei destinatari, ai servizi di abbonamento, ai flussi di lavoro di targeting esistenti e alle scelte rapide per la creazione di questi elementi.

Gli elenchi sono disponibili tramite **[!UICONTROL Lists]** nella sezione a sinistra del **[!UICONTROL Profiles and Targets]** di rete.
...
Ogni sezione sulle funzionalità di base ti consente di sfogliare tra gli elementi disponibili. A questo scopo, utilizza le scelte rapide nella **[!UICONTROL Browsing]** sezione. Il **[!UICONTROL Other choices]** consente di accedere a tutte le altre pagine, indipendentemente dall’ambiente.

Puoi creare un nuovo elemento (consegna, applicazione web, flusso di lavoro, ecc.) utilizzo delle scelte rapide in **[!UICONTROL Create]** a sinistra dello schermo. Utilizza il **[!UICONTROL Create]** sopra l’elenco per aggiungere nuovi elementi all’elenco.
...
Utilizzando i campi di input con un elenco a discesa, puoi immettere un valore di enumerazione, che può essere memorizzato e quindi offerto come opzione nell’elenco a discesa. Ad esempio, nel **[!UICONTROL City]** campo del **[!UICONTROL General]** di un profilo destinatario, puoi accedere a Londra. Quando premi Invio per confermare questo valore, viene visualizzato un messaggio in cui viene richiesto se desideri salvare questo valore per l’enumerazione associata al campo.
...
>[!NOTE]
>
>Le enumerazioni (note anche come &quot;elenchi dettagliati&quot;) sono gestite dall’amministratore tramite **[!UICONTROL Administration > Platform > Enumerations]** sezione. Per ulteriori informazioni, consulta [Gestione delle enumerazioni](../../platform/using/managing-enumerations.md).

### Unità predefinite {#default-units}

Nei campi che esprimono una durata (ad esempio periodo di validità delle risorse di una consegna, scadenza dell’approvazione per un’attività, ecc.), il valore può essere espresso come segue **unità**:
