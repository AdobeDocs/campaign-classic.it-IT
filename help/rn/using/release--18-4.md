...
* Risolto un problema durante la sincronizzazione degli spazi di offerta dall&#39;istanza di marketing all&#39;istanza di interazione. (NEO-11162)
...
* È stato risolto un problema nell&#39; **[!UICONTROL Survey Result]** attività quando si utilizzavano i dati memorizzati in XML. Il rapporto non veniva visualizzato correttamente. (NEO-10816)
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>Adobe Campaign ora fornisce l'elenco dei profili attivi, aggiornati mensilmente tramite un flusso di lavoro dedicato.</p> <p>Per ulteriori informazioni, consulta la documentazione <a href="../../platform/using/about-profiles.md#active-profiles"></a>dettagliata.</p> </td> 
  </tr> 
  <tr> 
...
* L&#39;espansione di entità esterne è ora disattivata per impedire potenziali attacchi da utenti non autenticati. (NEO-10173)
...
* **Generazione** automatica sequenza: Il meccanismo di generazione ID è stato migliorato per aumentare la durata delle istanze Campaign con grandi volumi di oggetti. Per ulteriori informazioni, consultare questa [nota tecnica](https://helpx.adobe.com/campaign/kb/sequence_auto_generation.html).
...
* È stata ripristinata la possibilità per gli amministratori di impostare o reimpostare la password di qualsiasi operatore. A questo scopo, fare clic con il pulsante destro del mouse su un operatore, selezionare **[!UICONTROL Actions]** > **[!UICONTROL Reset password]** e impostare la nuova password dell&#39;operatore. È consigliabile che gli operatori cambino la password al primo riconnessione. Per ulteriori informazioni, consulta la documentazione [](../../production/using/lost-password.md)dettagliata.
* Per supportare la nuova funzione di multitenanza in Adobe Target, ora è possibile aggiungere un nuovo parametro &quot;at_property&quot; agli URL quando si configurano opzioni e account esterni per l&#39;integrazione con Target. Il valore da utilizzare per questo parametro può essere trovato in Adobe Target e sarà utilizzato da Campaign per eseguire chiamate a Target. Per ulteriori informazioni, consulta la documentazione [](../../integrations/using/inserting-a-dynamic-image.md)dettagliata.
* Ora puoi specificare una pagina di destinazione predefinita da aprire quando fai clic su un&#39;immagine gestita da Adobe Target. In precedenza, facendo clic su tale immagine si passava al set di immagini predefinito al momento della creazione del messaggio e-mail. Per ulteriori informazioni, consulta la documentazione [](../../integrations/using/inserting-a-dynamic-image.md)dettagliata.
* Aggiunta della casella **Abilita tracce** SMPP nell&#39;account esterno per forzare l&#39;output delle tracce. Per ulteriori informazioni, consulta la documentazione [](../../delivery/using/sms-channel.md#creating-an-smpp-external-account)dettagliata.
...
* È stato risolto un problema a causa del quale i collegamenti contenuti in un messaggio e-mail di approvazione potevano puntare a un URL di accesso errato, in particolare quando si utilizzava un tipo di accesso Federated ID. (NEO-9011)
...
* È stato risolto un problema che poteva verificarsi durante il caricamento di file ZIP tramite un server FTP o durante il caricamento di file con caratteri jolly nel nome del file.
* È stato risolto un problema relativo alle enumerazioni di tipo base esteso nelle risorse personalizzate esterne create in Campaign Standard.
